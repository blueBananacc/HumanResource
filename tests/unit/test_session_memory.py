...
class TestFinalizeSession:
    ...
    @patch("human_resource.agents.orchestrator._get_compressor")
    @patch("human_resource.agents.orchestrator._get_session_memory")
    def test_writes_episodic_memory(self, mock_sm, mock_comp, mock_ltm):
        """会话结束时应生成 episodic 摘要并写入 mem0。"""
        from human_resource.agents.orchestrator import finalize_session

        session = Session(session_id="s1")
        session.messages = [
            SessionMessage(role="user", content="年假还有几天"),
            SessionMessage(role="assistant", content="您还有5天年假"),
        ]
        sm = MagicMock()
        ...
        compressor = MagicMock()
        compressor.summarize_text.return_value = "用户询问了年假余额"
        mock_comp.return_value = compressor

        ltm = MagicMock()
        mock_ltm.return_value = ltm

        finalize_session("s1", "u1")

        ltm.add.assert_called_once()
        call_args = ltm.add.call_args
    ...
    @patch("human_resource.agents.orchestrator._get_compressor")
    @patch("human_resource.agents.orchestrator._get_session_memory")
    def test_includes_stored_summary(self, mock_sm, mock_comp, mock_ltm):
        """有存储摘要时应包含在 episodic 文本中。"""
        from human_resource.agents.orchestrator import finalize_session

        session = Session(session_id="s1", summary="之前的对话摘要")
        session.messages = [
            SessionMessage(role="user", content="最近的问题"),
        ]
        sm = MagicMock()
        ...
        compressor = MagicMock()
        compressor.summarize_text.return_value = "完整摘要"
        mock_comp.return_value = compressor
        ...
        # summarize_text 应接收包含已有摘要的文本
        call_text = compressor.summarize_text.call_args[0][0]
        assert "之前的对话摘要" in call_text

    @patch("human_resource.agents.orchestrator._get_session_memory")
    def test_empty_session_skipped(self, mock_sm):
        """空会话不写入 episodic 记忆。"""
        from human_resource.agents.orchestrator import finalize_session
    ...
    @patch("human_resource.agents.orchestrator._get_compressor")
    @patch("human_resource.agents.orchestrator._get_session_memory")
    def test_ltm_failure_graceful(self, mock_sm, mock_comp, mock_ltm):
        """mem0 写入失败时不崩溃。"""
        from human_resource.agents.orchestrator import finalize_session

        session = Session(session_id="s1")
        session.messages = [
            SessionMessage(role="user", content="问题"),
        ]
        sm = MagicMock()
        sm.get_or_create.return_value = session
        mock_sm.return_value = sm
        ...
        compressor = MagicMock()
        compressor.summarize_text.return_value = "摘要"
        mock_comp.return_value = compressor

        ltm = MagicMock()
        ltm.add.side_effect = Exception("mem0 连接失败")
        mock_ltm.return_value = ltm
