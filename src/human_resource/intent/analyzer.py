...
可参考的意图类别：
- info_query: HR 信息查询（政策问答、流程咨询等，如年假政策、薪资规定、请假流程、入职流程等）
...
示例：
用户：公司年假是怎么计算的？
输出：理由：用户询问年假计算规则，属于HR政策问题。意图为：info_query。
---
消息：我想查一下我的年假额度和今年的调休规则
输出：理由：用户查询个人年假额度（员工信息）并询问调休规则（政策信息）。意图为：employee_lookup + info_query。
---
...
输出：理由：虽然上下文中有技能执行记录，但用户已明显切换话题到请假流程，不再继续技能任务。意图为：info_query。
